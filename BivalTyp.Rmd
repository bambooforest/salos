...
The only information on languages in our `valency` is some kind of ID under `language_no`, but what is language number `1`? 
...
Let's look at some valency details for a langauge or two. `filter()` allows you to access only those data which fullfil the specified conditions (e.g. `==` means it has to exactly matching `"English"`)
```{r, message = FALSE}
