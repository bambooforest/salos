...
If you have not installed these libraries before, then you need to run the `install.packages()` command **once** before you load them above, i.e., `install.packages('tidyverse')` in the Console below. 

Or you can use the menu above in RStudio: Click on Tools > Install Packages.
...
There are numerous ways to load data into R. The most common for text-based tables are:
...
For loading Excel spreadsheets, we can use `tidyverse` with the `[readxl](https://readxl.tidyverse.org)` package, e.g.:
